...
```{r plot of frequency of categorical vars}
#processed_fake <-processed_fake [-which(processed_fake $type %in% "bs"),]
library(ggplot2)
...
ggplot(processed_fake , aes(x = processed_fake$crawled_wekdays, fill = processed_fake$type))+ 
  geom_bar(alpha = 0.5) +
  ggtitle("Histogram Plot:  Country") + 
  labs(x = "Country", y = "Counts") + 
  guides(fill=guide_legend(title="Country")) +

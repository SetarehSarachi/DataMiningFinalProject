...
#final.bait_satire <- do.combine(bait_satire[,-1], actual.vs.predicted.bait_satire)
...
final.bait_bias <- do.combine(bait_bias[,-1], actual.vs.predicted.bait_bias)
final.bait_bias<- cbind(bait_bias$X,final.bait_bias)
```

```{r algo_bait_bs}
actual.vs.predicted.bait_bs = data.frame(actual = bait_bs$type)
final.bait_bs <- do.combine(bait_bs[,-1], actual.vs.predicted.bait_bs)
```
...
final.bait_conspiracy <- do.combine(bait_conspiracy[,-1], actual.vs.predicted.bait_conspiracy)
```

```{r algo_bait_fake}
actual.vs.predicted.bait_fake = data.frame(actual = bait_fake$type)
final.bait_fake <- do.combine(bait_fake[,-1], actual.vs.predicted.bait_fake)
```

```{r algo_bait_hate}
actual.vs.predicted.bait_hate = data.frame(actual = bait_hate$type)
final.bait_hate<- do.combine(bait_hate[,-1], actual.vs.predicted.bait_hate)
```

```{r algo_bait_state}
actual.vs.predicted.bait_state = data.frame(actual = bait_state$type)
final.bait_state <- do.combine(bait_state[,-1], actual.vs.predicted.bait_state)
```

```{r algo_bs_bias}
actual.vs.predicted.bs_bias = data.frame(actual = bs_bias$type)
final.bs_bias <- do.combine(bs_bias[,-1], actual.vs.predicted.bs_bias)
```

```{r algo_bias_conspiracy}
actual.vs.predicted.bias_conspiracy = data.frame(actual = bias_conspiracy$type)
final.bias_conspiracy <- do.combine(bias_conspiracy[,-1], actual.vs.predicted.bias_conspiracy)
```

```{r algo_bias_fake}
actual.vs.predicted.bias_fake = data.frame(actual = bias_fake$type)
final.bias_fake <- do.combine(bias_fake[,-1], actual.vs.predicted.bias_fake)
```

```{r algo_bias_hate}
actual.vs.predicted.bias_hate = data.frame(actual = bias_hate$type)
final.bias_hate <- do.combine(bias_hate[,-1], actual.vs.predicted.bias_hate)
```

```{r algo_bias_junksci}
actual.vs.predicted.bias_junksci = data.frame(actual = bias_junksci$type)
final.bias_junksci <- do.combine(bias_junksci[,-1], actual.vs.predicted.bias_junksci)
```

```{r algo_bias_satire}
actual.vs.predicted.bias_satire = data.frame(actual = bias_satire$type)
final.bias_satire <- do.combine(bias_satire[,-1], actual.vs.predicted.bias_satire)
```

```{r algo_bias_state}
actual.vs.predicted.bias_state= data.frame(actual = bias_state$type)
final.bias_state <- do.combine(bias_state[,-1], actual.vs.predicted.bias_state)
```

```{r algo_bs_conspiracy}
actual.vs.predicted.bs_conspiracy = data.frame(actual = bs_conspiracy$type)
final.bs_conspiracy <- do.combine(bs_conspiracy[,-1], actual.vs.predicted.bs_conspiracy)
```

```{r algo_bs_fake}
actual.vs.predicted.bs_fake = data.frame(actual = bs_fake$type)
final.bs_fake <- do.combine(bs_fake[,-1], actual.vs.predicted.bs_fake)
```

```{r algo_bs_hate}
actual.vs.predicted.bs_hate = data.frame(actual = bs_hate$type)
final.bs_hate <- do.combine(bs_hate[,-1], actual.vs.predicted.bs_hate)
```

```{r algo_bs_junksci}
actual.vs.predicted.bs_junksci = data.frame(actual = bs_junksci$type)
final.bs_junksci <- do.combine(bs_junksci[,-1], actual.vs.predicted.bs_junksci)
```

```{r algo_bs_satire}
actual.vs.predicted.bs_satire = data.frame(actual = bs_satire$type)
final.bs_satire <- do.combine(bs_satire[,-1], actual.vs.predicted.bs_satire)
```

```{r algo_bs_state}
actual.vs.predicted.bs_state = data.frame(actual = bs_state$type)
final.bs_state <- do.combine(bs_state[,-1], actual.vs.predicted.bs_state)
```

```{r algo_conspiracy_fake}
actual.vs.predicted.conspiracy_fake= data.frame(actual = conspiracy_fake$type)
final.conspiracy_fake <- do.combine(conspiracy_fake[,-1], actual.vs.predicted.conspiracy_fake)
```

```{r algo_conspiracy_hate}
actual.vs.predicted.conspiracy_hate = data.frame(actual =conspiracy_hate$type)
final.conspiracy_hate <- do.combine(conspiracy_hate[,-1], actual.vs.predicted.conspiracy_hate)
```

```{r algo_conspiracy_junksci}
actual.vs.predicted.conspiracy_junksci = data.frame(actual = conspiracy_junksci$type)
final.conspiracy_junksci <- do.combine(conspiracy_junksci[,-1], actual.vs.predicted.conspiracy_junksci)
```

```{r algo_conspiracy_satire}
actual.vs.predicted.conspiracy_satire = data.frame(actual = conspiracy_satire$type)
final.conspiracy_satire <- do.combine(conspiracy_satire[,-1], actual.vs.predicted.conspiracy_satire)
```

```{r algo_conspiracy_state}
actual.vs.predicted.conspiracy_state = data.frame(actual = conspiracy_state$type)
final.conspiracy_state <- do.combine(conspiracy_state[,-1], actual.vs.predicted.conspiracy_state)
```

```{r algo_fake_hate}
actual.vs.predicted.fake_hate = data.frame(actual = fake_hate$type)
final.fake_hate <- do.combine(fake_hate[,-1], actual.vs.predicted.fake_hate)
```

```{r algo_fake_junksci}
actual.vs.predicted.fake_junksci = data.frame(actual = fake_junksci$type)
final.fake_junksci <- do.combine(fake_junksci[,-1], actual.vs.predicted.fake_junksci)
```

```{r algo_fake_satire}
actual.vs.predicted.fake_satire = data.frame(actual = fake_satire$type)
final.fake_satire <- do.combine(fake_satire[,-1], actual.vs.predicted.fake_satire)
```

```{r algo_fake_state}
actual.vs.predicted.fake_state= data.frame(actual = fake_state$type)
final.fake_state <- do.combine(fake_state[,-1], actual.vs.predicted.fake_state)
```

```{r algo_hate_junksci}
actual.vs.predicted.hate_junksci = data.frame(actual = hate_junksci$type)
final.hate_junksci <- do.combine(hate_junksci[,-1], actual.vs.predicted.hate_junksci)
```

```{r algo_hate_satire}
actual.vs.predicted.hate_satire = data.frame(actual = hate_satire$type)
final.hate_satire<- do.combine(hate_satire[,-1], actual.vs.predicted.hate_satire)
```

```{r algo_hate_state}
actual.vs.predicted.hate_state = data.frame(actual = hate_state$type)
final.hate_state <- do.combine(hate_state[,-1], actual.vs.predicted.hate_state)
```

```{r algo_junksci_satire}
actual.vs.predicted.junksci_satire = data.frame(actual = junksci_satire$type)
final.junksci_satire<- do.combine(junksci_satire[,-1], actual.vs.predicted.junksci_satire)
```

```{r algo_junksci_state}
actual.vs.predicted.junksci_state = data.frame(actual = junksci_state$type)
final.junksci_state <- do.combine(junksci_state[,-1], actual.vs.predicted.junksci_state)
```

```{r algo_satire_state}
actual.vs.predicted.satire_state = data.frame(actual =satire_state$type)
final.satire_state <- do.combine(satire_state[,-1], actual.vs.predicted.satire_state)
```
